...
def get_service_list(kwargs):
	try:
		...
		for service in services:
			banner = frappe.get_list(
				"Banner", filters={"name": service}, fields=["banner_background_image", "banner_name"]
			)
			for b in banner:
				banner_data = b
			service_details_list = frappe.get_all(
				"Service Details", filters={"parent": service}, pluck="service_detail", order_by="sequence"
			)
			service_detail = []
			for service_details in service_details_list:
				get_service_details = frappe.get_all(
					"Service Details Master",
					filters={"name": service_details},
					fields=["heading", "description", "label"],
				)
				service_detail.extend(get_service_details)
			service_desc = frappe.get_list(
				"Services Master",
			...
			for service in service_desc:
				desc = service
			details = {
				"name": banner_data["banner_name"],
				"banner_image": banner_data["banner_background_image"],
				"slug": desc["slug"],
...
def get_service_details(kwargs):
	try:
		...
		for d in service_desc:
			...
			service_details_list = frappe.get_all(
				"Service Details", filters={"parent": d.name}, pluck="service_detail", order_by="sequence"
			)
...
def home_page_services(kwargs):
	try:
		...
		for service in services:
			...
			service_details_list = frappe.get_all(
				"Service Details",
				filters={"parent": service, "show_on_website": 1},
				fields=["service_detail", "show_on_website"],
				order_by="sequence",
			)
			service_detail = []
			for service_details in service_details_list:
				get_service_details = frappe.get_all(
					"Service Details Master",
					filters={"name": service_details.service_detail},
					fields=["heading", "description", "label"],
				)
				service_detail.extend(get_service_details)
			service_desc = frappe.get_list(

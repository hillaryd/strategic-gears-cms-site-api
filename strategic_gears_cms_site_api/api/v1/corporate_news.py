...
def get_corporate_news_data(kwargs):
	try:
		...
		corporate_news = frappe.get_all(
			"Banner",
			filters={"banner_name": "CORPORATE NEWS"},
			fields=["banner_name", "banner_text", "banner_height", "banner_font_size", "banner_alignment"],
		)
		corporate_news_articles = frappe.get_all(
			"Corporate News Articles",
			fields=[
				"article_heading",
				"show_data_on_website",
				"article_image",
				"article_description",
				"slug",
			],
		...
		data_req = {"banner_data": {}, "corporate_articles_list": []}
...
def get_corporate_news_article_details(kwargs):
	try:
		...
		articles = frappe.get_list(
			"Corporate News Articles",
			filters={"slug": corporate_news_article},
			fields=[
				"name",
				"banner",
				"article_heading",

...
def get_about_strategic_gears(kwargs):
	try:
		...
		banner_data = frappe.db.get_list(
			"Banner",
		...
		data["overview_data"] = {
			"overview_heading": doc.overview_heading,
		...
		for item in doc.organizational_purpose_and_values:
			overview_value = {"value_heading": item.heading, "value_description": item.description}
			data["overview_data"]["overview_values"].append(overview_value)

		data["services_data"] = {
			"services_heading": doc.services_heading,
		...
		for item in doc.services_list:
			service_data = frappe.get_all(
				"Service Details",
				filters={"parent": item.services_list},
				fields=["service_detail"],
			)
			service_master = frappe.get_list(
				"Services Master",
		...
		data["achievements_data"] = {"achievements_text": ""}

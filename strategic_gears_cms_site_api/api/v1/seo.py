...
def get_meta_tags(kwargs):
	...
	meta = frappe.db.get_value(
		...
		["page_name", "meta_title", "robots", "description"],
		as_dict=1,
	)
...
def sub_category_urls():
	parent_categories = frappe.get_all(
		"Category", {"slug": ["is", "set"], "is_group": ["is", "set"]}, ["slug", "name"]
	)
	result = []
	for parent_category in parent_categories:
		...
		SUB_CATEGORIES = frappe.get_all(
			"Category",
			{"slug": ["is", "set"], "old_parent": parent_category.name, "parent_category": ["is", "set"]},
			["slug", "name"],
		)
...
def corporate_news_urls():
	corporate_news_articles = frappe.get_all(
		"Corporate News Articles", {"slug": ["is", "set"]}, ["slug", "name"]
	)
	result = []
